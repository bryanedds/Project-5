...
type [<ReferenceEquality; SymbolicExpansion>] Character =
    ...
    static member private computeTraversalAnimations rotation linearVelocity angularVelocity character =
        ...
        | NormalState ->
            ...
            let animations =
                [Animation.make 0.0f None "Armature|Idle" Loop 1.0f 0.5f None]
            let animations =
    ...
    static member private updateState time character =
        ...
        | InjuryState injury ->
            let actionState =
                ...
                let injuryTime = match character.CharacterType with Player -> 0.5f | Enemy -> 0.667f
    ...
    static member private updateAttackedCharacters time character =
        ...
        | AttackState attack ->
            let localTime = time - attack.AttackTime
            let attack =
                if localTime > 0.92f && localTime < 0.94f
                then { attack with AttackedCharacters = Set.empty } // reset attack tracking at start of buffered attack
                else attack
            if localTime >= 0.333f && localTime < 0.5f || localTime >= 1.3f && localTime < 1.4f then
    ...
    static member updateInputKey time keyboardKeyData character =
        ...
        | Player ->
            ...
            if keyboardKeyData.KeyboardKey = KeyboardKey.Space && not keyboardKeyData.Repeated then
                ...
                if sinceJump >= 0.2f && sinceOnGround < 0.17f && character.ActionState = NormalState then
            ...
            // attacking
                ...
                let character =
                    ...
                    | AttackState attack ->
                        let localTime = time - attack.AttackTime
                        if localTime > 0.17f && not attack.FollowUpBuffered
