...
type CharacterDispatcher (character : Character) =
    ...
    override this.Message (character, message, entity, world) =
        ...
        | UpdateInputKey keyboardKeyData ->
            let (jump, character) = Character.updateInputKey world.ClockTime keyboardKeyData character
            withSignals (if jump then [Jump] else []) character

        | Update ->

            // update character
            let time = world.ClockTime
            ...
            let signals = match character.ActionState with WoundState wound when wound.WoundTime = world.ClockTime - 1.0f -> PublishDie :> Signal :: signals | _ -> signals
